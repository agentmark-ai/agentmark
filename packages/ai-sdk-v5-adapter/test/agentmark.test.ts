...
describe("AgentMark Integration", () => {
  ...
  describe("AIAdapter Integration", () => {
    ...
    it("should adapt object prompts with tools for Vercel AI SDK v5", async () => {
      const fixturesDir = path.resolve(__dirname, "./fixtures");
      const fileLoader = new FileLoader(fixturesDir);
      const mockModelFn = vi.fn().mockImplementation((modelName) => ({
        name: modelName,
      ...
      const mockCalculatorTool = {
        ...
        inputSchema: { type: "object" as const, properties: { expression: { type: "string" as const } } },
      ...
      const agentMark = createAgentMarkClient<TestPromptTypes>({
        loader: fileLoader,
        modelRegistry,
        tools: { calculator: mockCalculatorTool as unknown as Tool },
      });
