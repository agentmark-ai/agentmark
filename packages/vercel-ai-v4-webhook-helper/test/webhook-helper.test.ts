...
describe("WebhookHelper", () => {
  ...
  describe("runPrompt", () => {
    ...
    it("should handle object_config prompts", async () => {
      ...
      expect(mockAgentMarkClient.loadObjectPrompt).toHaveBeenCalledWith(baseEvent.prompt);
      expect(mockPrompt.formatWithTestProps).toHaveBeenCalledWith({});
      expect(mockInferenceAdapter.runObjectPrompt).toHaveBeenCalledWith(formattedInput, options);
    ...
    it("should handle text_config prompts", async () => {
      ...
      expect(getFrontMatter).toHaveBeenCalledWith(baseEvent.prompt);
      expect(mockAgentMarkClient.loadTextPrompt).toHaveBeenCalledWith(baseEvent.prompt);
      expect(mockPrompt.formatWithTestProps).toHaveBeenCalledWith({});
      expect(mockInferenceAdapter.runTextPrompt).toHaveBeenCalledWith(formattedInput, undefined);
      expect(result).toEqual(expectedResponse);
    });
    ...
    it("should handle image_config prompts", async () => {
      ...
      expect(getFrontMatter).toHaveBeenCalledWith(baseEvent.prompt);
      expect(mockAgentMarkClient.loadImagePrompt).toHaveBeenCalledWith(baseEvent.prompt);
      expect(mockPrompt.formatWithTestProps).toHaveBeenCalledWith({});
      expect(mockInferenceAdapter.runImagePrompt).toHaveBeenCalledWith(formattedInput);
      expect(result).toEqual(expectedResponse);
    });
    ...
    it("should handle speech_config prompts", async () => {
      ...
      expect(getFrontMatter).toHaveBeenCalledWith(baseEvent.prompt);
      expect(mockAgentMarkClient.loadSpeechPrompt).toHaveBeenCalledWith(baseEvent.prompt);
      expect(mockPrompt.formatWithTestProps).toHaveBeenCalledWith({});
      expect(mockInferenceAdapter.runSpeechPrompt).toHaveBeenCalledWith(formattedInput);
      expect(result).toEqual(expectedResponse);
    });
    ...
    it("should throw error for invalid prompt", async () => {
      ...
      await expect(webhookHelper.runPrompt(baseEvent)).rejects.toThrow("Invalid prompt");
    });
  });
  ...
  describe("runDataset", () => {
    ...
    it("should throw error when loader is not found", async () => {
      ...
      await expect(webhookHelper.runDataset(baseEvent)).rejects.toThrow("Loader not found");
    });
    ...
    it("should handle object_config dataset runs", async () => {
      const frontmatter = {
    ...
    it("should throw error for invalid dataset prompt", async () => {
      ...
      await expect(webhookHelper.runDataset(baseEvent)).rejects.toThrow("Invalid prompt");
    });
    ...
    it("should handle multiple dataset items correctly", async () => {
      ...
      const mockDataset = [
        {
          formatted: { messages: [{ role: "user", content: "Item 1" }], experimental_telemetry: { metadata: {} } },
          dataset: { input: "Item 1", expected_output: "Response 1" },
        },
        {
          formatted: { messages: [{ role: "user", content: "Item 2" }], experimental_telemetry: { metadata: {} } },
      ...
      mockInferenceAdapter.runTextPrompt
        .mockResolvedValueOnce({ type: "text", result: "Response 1", usage: { totalTokens: 5 } })
        .mockResolvedValueOnce({ type: "text", result: "Response 2", usage: { totalTokens: 8 } });
      ...
      expect(mockInferenceAdapter.runTextPrompt).toHaveBeenNthCalledWith(1,
        expect.objectContaining({
          experimental_telemetry: expect.objectContaining({
            metadata: expect.objectContaining({
              dataset_item_name: 0,
      ...
      expect(mockInferenceAdapter.runTextPrompt).toHaveBeenNthCalledWith(2,
        expect.objectContaining({
          experimental_telemetry: expect.objectContaining({
            metadata: expect.objectContaining({
              dataset_item_name: 1,
