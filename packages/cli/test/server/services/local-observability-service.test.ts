  ...
  describe('getTraces', () => {
    ...
    it('should return traces in correct format when traces exist', async () => {
      ...
      expect(result.traces[0]).toEqual({
        ...
        tokens: 150,
        spanCount: 3,
        // The service now propagates the tags column (parsed from the
        // SQL `json_group_array` output) so the wire-mapper can emit it
        // on the response. With no tags in the test row it's `[]`.
        tags: [],
      });
    });
