...
const dev = async (options: { apiPort?: number; webhookPort?: number; appPort?: number; forward?: boolean; ui?: boolean } = {}) => {
  ...
  if (projectLanguage === "python") {
    ...
    if (!fs.existsSync(pythonConfigPath)) {
      ...
      console.error('Run `agentmark dev` from your AgentMark Python project root. If the client is not set up yet, ask your coding agent to "Set up AgentMark in this project," or follow https://docs.agentmark.co/getting-started/client-setup.');
      process.exit(1);
    }
  } else {
    ...
    if (!fs.existsSync(configPath)) {
      ...
      console.error('Run `agentmark dev` from your AgentMark project root. If the client is not set up yet, ask your coding agent to "Set up AgentMark in this project," or follow https://docs.agentmark.co/getting-started/client-setup.');
      process.exit(1);
    }
  }
  ...
  if (projectLanguage === "python") {
    ...
    if (fs.existsSync(customPythonDevServer)) {
      ...
      console.error('Expected .agentmark/dev_server.py (or dev_server.py at your project root).');
      console.error('This file is created when you set up the AgentMark client — ask your coding agent to "Set up AgentMark in this project," or follow https://docs.agentmark.co/getting-started/client-setup.');
      process.exit(1);
    }
  } else {
    ...
    if (fs.existsSync(customDevServer)) {
      ...
      console.error('Expected dev-entry.ts at your project root (or .agentmark/dev-entry.ts).');
      console.error('This file is created when you set up the AgentMark client — ask your coding agent to "Set up AgentMark in this project," or follow https://docs.agentmark.co/getting-started/client-setup.');
      process.exit(1);
    }
  }
