...
export const createExampleApp = async (modelProvider: string, model: string, useCloud: string = 'cloud') => {
  try {
...
---`
    ...
    const rulesForPrompt = fs.readFileSync(path.join(editorRulesDir, 'agentmark-prompt.md'), 'utf8');
    const cursorRuleForPrompt = `${cursorRuleMetadata}\n\n${rulesForPrompt}`;
    fs.writeFileSync(path.join(targetRulesDir, 'prompt-guidelines.mdc'), cursorRuleForPrompt);
    
    // Copy agentmark-dataset.md to .cursor/rules/dataset-guidelines.mdc
    const rulesForDataset = fs.readFileSync(path.join(editorRulesDir, 'agentmark-dataset.md'), 'utf8');
    const cursorRuleForDataset = `${cursorRuleMetadata}\n\n${rulesForDataset}`;
    fs.writeFileSync(path.join(targetRulesDir, 'dataset-guidelines.mdc'), cursorRuleForDataset);
    ...
    fs.writeFileSync('./index.ts', getIndexFileContent(modelProvider, model, useCloud));
    ...
    if (useCloud === 'cloud') {
      console.log('1. Update the .env file with your AgentMark Cloud and API credentials');
      ...
      console.log('2. Run "npm start" to execute the example and see the results locally');
    }
