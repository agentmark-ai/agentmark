...
import {
  ...
  displayAudioInBrowser,
  createImageFile,
  createAudioFile,
  createClickableLink,
  printFilePath,
} from "../utils/web-viewer";
...
const executeSpeechPropsPrompt = async (input: any) => {
  ...
  try {
    ...
    console.log(
      `Generated audio: ${result.audio.mimeType} (${sizeKB}KB) - Opening in browser...`
    );
  } catch (error: any) {
  ...
  try {
    while (true) {
      ...
      try {
        ...
        rowTable.push([index.toString(), input, expectedOutput, text]);
        ...
        console.error(
          `❌ Error generating text for entry ${index}: ${error.message}`
        );
  ...
  try {
    while (true) {
      ...
      try {
        ...
        rowTable.push([index.toString(), input, expectedOutput, aiResult]);
        ...
        console.error(
          `❌ Error generating object for entry ${index}: ${error.message}`
        );
  ...
  try {
    while (true) {
      ...
      try {
        ...
        const totalSizeKB = result.images.reduce(
          (sum, img) => sum + Math.round((img.base64.length * 0.75) / 1024),
          0
        );
        ...
        rowTable.push([index.toString(), input, expectedOutput, resultText]);
        ...
        console.error(
          `❌ Error generating images for entry ${index}: ${error.message}`
        );
  ...
  try {
    while (true) {
      ...
      try {
        ...
        console.error(
          `❌ Error generating audio for entry ${index}: ${error.message}`
        );
...
const runPrompt = async (filepath: string, options: RunPromptOptions) => {
  ...
  if (frontmatter?.metadata) {
    console.log(
      "Warning: Old format detected. Please consider updating to the new format."
    );
  }
  ...
  const agentMark = createAgentMarkBuilder().withLoader(fileLoader).build();
