...
const dev = async (options: { port?: number; runnerPort?: number } = {}) => {
  ...
  if (!fs.existsSync(customDevServer)) {
    ...
    // Convert adapter name to runner class name (e.g., "vercel-ai-v4" -> "VercelAdapterRunner")
    const runnerClassName = adapterName.split('-').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join('') + 'AdapterRunner';
