...
async function generateTypeDefinitions(prompts: PromptFrontmatter[]): Promise<string> {
  let interfaces: string[] = [];
  const headerComment = `// Auto-generated types from Puzzlet
...
type ${name}Out = string`
  ...
  output += `export default interface PuzzletTypes {
${typeMapping.join(',\n')}
}\n`;
...
async function fetchPromptsFrontmatter(options: { local?: number, rootDir?: string }): Promise<PromptFrontmatter[]> {
  if (options.local) {
    const baseUrl = `http://localhost:${options.local}`;
    try {
      const pathsResponse = await fetch(`${baseUrl}/v1/prompts`);
      if (!pathsResponse.ok) {
        throw new Error(`Failed to fetch prompt paths: ${pathsResponse.statusText}`);
      }
      
      const { paths } = await pathsResponse.json();
      
      return Promise.all(paths.map(async (promptPath: string) => {
        const templateResponse = await fetch(`${baseUrl}/v1/templates?path=${promptPath}`);
        if (!templateResponse.ok) {
          throw new Error(`Failed to fetch template ${promptPath}: ${templateResponse.statusText}`);
        }
