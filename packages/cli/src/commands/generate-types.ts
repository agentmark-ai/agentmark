...
function jsonSchemaToJsdocParam(
  propName: string,
  propSchema: any,
  parentPath = ""
): string {
  ...
  return ` * @param {${type}} ${fullPath}${description ? ` - ${description}` : ""}`;
}

function generateJsdocForSchema(
  promptPath: string,
  input_schema: any
): string {
  const functionName = getInterfaceName(promptPath);
  const jsDocLines: string[] = [];
  
  jsDocLines.push("/**");
  jsDocLines.push(` * ${promptPath.replace(/\.prompt\.mdx$/, "")} - AgentMark Prompt`);
  jsDocLines.push(" *");
  jsDocLines.push(` * This is an auto-generated JSDoc for the ${promptPath} prompt file.`);
  jsDocLines.push(" * Use this documentation to understand the expected input parameters.");
  jsDocLines.push(" *");
  
  if (input_schema && input_schema.properties) {
    const required = input_schema.required || [];
    
    Object.entries(input_schema.properties).forEach(([propName, propSchema]: [string, any]) => {
      const paramLine = jsonSchemaToJsdocParam(propName, propSchema);
      jsDocLines.push(paramLine);
    });
    
    if (required.length > 0) {
      jsDocLines.push(" *");
      jsDocLines.push(` * @required ${required.join(", ")}`);
    }
  } else {
    jsDocLines.push(" * @param {Object} props - The props object (no schema defined)");
  }
  
  jsDocLines.push(" *");
  jsDocLines.push(` * @example`);
  jsDocLines.push(` * // Example usage in MDX:`);
  jsDocLines.push(` * import { ${functionName} } from './agentmark.jsdoc.js';`);
  jsDocLines.push(` * `);
  
  if (input_schema && input_schema.properties) {
    jsDocLines.push(` * // With these props:`);
    Object.entries(input_schema.properties).forEach(([propName, propSchema]: [string, any]) => {
      let exampleValue = "value";
      if (propSchema.type === "string") exampleValue = `"example ${propName}"`;
      else if (propSchema.type === "number" || propSchema.type === "integer") exampleValue = "42";
      else if (propSchema.type === "boolean") exampleValue = "true";
      else if (propSchema.type === "array") exampleValue = "[]";
      else if (propSchema.type === "object") exampleValue = "{}";
      
      jsDocLines.push(` * // ${propName}: ${exampleValue}`);
    });
  }
  
  jsDocLines.push(" */");
  jsDocLines.push(`export function ${functionName}(props) {`);
  jsDocLines.push("  // This function serves as JSDoc documentation");
  jsDocLines.push("  // The actual prompt logic is handled by AgentMark");
  jsDocLines.push("  return props;");
  jsDocLines.push("}");
...
async function generateJsdocDefinitions(prompts: PromptFrontmatter[]): Promise<string> {
  const headerComment = `/**
 * Auto-generated JSDoc from AgentMark .prompt.mdx files
 * 
 * This file provides JSDoc documentation for all .prompt.mdx files in your project.
 * Each function represents a prompt and documents its expected input parameters.
 * 
 * To use this in your .prompt.mdx files:
 * 1. Import the relevant function: import { YourPromptName } from './agentmark.jsdoc.js'
 * 2. The function serves as documentation - AgentMark handles the actual prompt logic
 * 3. Use your IDE's intellisense to see parameter documentation
 * 
 * @fileoverview AgentMark JSDoc Documentation
...
`;
  ...
  for (const prompt of prompts) {
    ...
    try {
      const jsdocDef = generateJsdocForSchema(promptPath, input_schema);
      ...
      const fallbackJsdoc = [
        "/**",
        ` * ${promptPath.replace(/\.prompt\.mdx$/, "")} - AgentMark Prompt`,
        " *",
        " * Error processing schema for this prompt.",
  ...
  // Add a helper function to the end
...
export function getAllPromptDocs() {
  return {
${prompts.map(prompt => {
  const functionName = getInterfaceName(prompt.path);
  return `    "${prompt.path}": ${functionName}`;
}).join(",\n")}
  };
}`;
