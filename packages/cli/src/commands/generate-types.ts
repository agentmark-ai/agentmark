...
const generateTypes = async ({ language, local, rootDir }: Options) => {
  if (language !== "typescript" && language !== "jsdoc") {
    console.error(
      `Unsupported language: ${language}. Only TypeScript and JSDoc are supported.`
    );
    return;
  }
  ...
  try {
    if (language === "jsdoc") {
      console.error("Generating JSDoc definitions...");
    } else {
    ...
    let output: string;
    if (language === "jsdoc") {
      output = await generateJSDocDefinitions(prompts);
    } else {
      output = await generateTypeDefinitions(prompts);
    }
