...
- A streamed prompt run that errors mid-stream now marks the prompt span ERROR, matching the non-streaming path. Previously the streaming run paths (text + object) reported the executor error to the caller as a wire error chunk but closed the span clean, so a failed production run looked successful in the trace (status OK). The streaming generators now capture the error and close the span with it — the wire error chunk is unchanged, only the span status. Mirrored in `prompt-core-python`. ([#835](https://github.com/agentmark-ai/agentmark/pull/835))

### 🧱 Updated Dependencies

- Updated @agentmark-ai/ui-components to 0.13.5
- Updated @agentmark-ai/prompt-core to 1.2.3

## 0.27.6 (2026-06-23)

### 🩹 Fixes

- Trace → dataset conversion now defaults a dataset row's `input` to the prompt's template **variables** (props) instead of the rendered chat messages — across the trace viewer's "Add to dataset" (ui-components `extractSpanInput` / `getSpanInputKind`) and the CLI's local `import_dataset_rows_from_traces` (`api-server`). ([#833](https://github.com/agentmark-ai/agentmark/pull/833))
...
- Updated @agentmark-ai/ui-components to 0.13.4

## 0.27.5 (2026-06-23)

### 🩹 Fixes

- Emit the prompt's folder-aware path on trace spans as a new `agentmark.prompt_path` attribute, across the TS and Python SDKs. ([#826](https://github.com/agentmark-ai/agentmark/pull/826))
...
- Updated @agentmark-ai/ui-components to 0.13.3
- Updated @agentmark-ai/api-schemas to 0.11.0
- Updated @agentmark-ai/prompt-core to 1.2.2
- Updated @agentmark-ai/api-types to 0.11.1
...
- Updated @agentmark-ai/ui-components to 0.13.1
- Updated @agentmark-ai/shared-utils to 0.9.1
...
- `agentmark init` now scaffolds the provider-agnostic client (`agentmark.client.ts` ([#799](https://github.com/agentmark-ai/agentmark/pull/799))
...
- `doctor` / `build` / `dev` now give an actionable error when a prompt's ([#797](https://github.com/agentmark-ai/agentmark/pull/797))
