...
class TestRunPromptUnsupportedTypes:
    ...
    async def test_throws_for_unrecognized_config_types(
        ...
        with patch.object(handler, "_get_frontmatter", return_value={"name": "unknown-prompt"}):
            with pytest.raises(ValueError, match=r"Invalid prompt: No recognized config type"):
                await handler.run_prompt(create_mock_ast())
...
class TestRunPromptErrorHandling:
    ...
    async def test_includes_trace_id_even_when_prompt_name_missing(
        ...
        with patch.object(handler, "_get_frontmatter", return_value={"text_config": {}}):
            with patch.object(handler, "_execute_query", return_value=mock_results):
                result = await handler.run_prompt(create_mock_ast())
