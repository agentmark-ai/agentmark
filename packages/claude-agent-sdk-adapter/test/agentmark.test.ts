...
describe("Claude Agent SDK Adapter Integration", () => {
  ...
  describe("ClaudeAgentAdapter - Text Prompts", () => {
    it("should adapt text prompts for Claude Agent SDK", async () => {
      ...
      expect(result.prompt).toContain("Hello, how are you?");
    ...
    it("should extract system prompt from messages", async () => {
      ...
      expect(result.options.systemPrompt).toBe("You are a helpful assistant.");
  ...
  describe("ClaudeAgentAdapter - Object Prompts", () => {
    it("should adapt object prompts with structured output", async () => {
      ...
      expect(result.options.outputFormat).toBeDefined();
      expect(result.options.outputFormat.type).toBe("json_schema");
      expect(result.options.outputFormat.schema).toBeDefined();
    });
  });
  ...
  describe("ClaudeAgentAdapter - Adapter Options", () => {
    ...
    it("should apply maxTurns from adapter options", async () => {
      ...
      expect(result.options.maxTurns).toBe(50);
    });
    ...
    it("should apply cwd from adapter options", async () => {
      ...
      expect(result.options.cwd).toBe("/custom/path");
    });
    ...
    it("should apply allowedTools and disallowedTools", async () => {
      ...
      expect(result.options.allowedTools).toEqual(["Read", "Write"]);
      expect(result.options.disallowedTools).toEqual(["Bash"]);
    });
  });

  describe("ClaudeAgentAdapter - Telemetry", () => {
    it("should include telemetry hooks when enabled", async () => {
      const fixturesDir = path.resolve(__dirname, "./fixtures");
      const fileLoader = new FileLoader(fixturesDir);
      const modelRegistry = ClaudeAgentModelRegistry.createDefault();
      ...
      expect(result.options.hooks?.SessionStart).toBeDefined();
      expect(result.options.hooks?.SessionEnd).toBeDefined();
      expect(result.options.hooks?.PreToolUse).toBeDefined();
      expect(result.options.hooks?.PostToolUse).toBeDefined();
    });

    it("should not include hooks when telemetry is disabled", async () => {
      const fixturesDir = path.resolve(__dirname, "./fixtures");
      const fileLoader = new FileLoader(fixturesDir);
      const modelRegistry = ClaudeAgentModelRegistry.createDefault();
      ...
      // No hooks should be present when telemetry is not enabled
  ...
  describe("ClaudeAgentAdapter - Tool Registry Integration", () => {
    it("should include MCP servers when tools are defined in prompt and registered", async () => {
      ...
      expect(result.options.mcpServers?.["prompt-tools"]).toBeDefined();
    });
