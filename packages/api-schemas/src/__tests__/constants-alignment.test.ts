...
import {
  ...
  ImportDatasetRowsResponseSchema,
...
describe('constants ↔ schema alignment', () => {
  ...
  describe('dataset and graph response schemas', () => {
    it('DatasetRowSchema should accept canonical dataset rows', () => {
      expect(() => DatasetRowSchema.parse({
        input: { prompt: 'hello' },
    ...
    it('ImportDatasetRowsFromTracesBodySchema should accept trace imports', () => {
      expect(() => ImportDatasetRowsFromTracesBodySchema.parse({
        trace_ids: ['trace-1'],
        mapping: {
          input: '$.input',
    ...
    it('ImportDatasetRowsFromSpansBodySchema should accept span imports', () => {
      expect(() => ImportDatasetRowsFromSpansBodySchema.parse({
        span_ids: ['span-1'],
      })).not.toThrow();
    });

    it('ImportDatasetRowsResponseSchema should accept mixed per-item results', () => {
      expect(() => ImportDatasetRowsResponseSchema.parse({
        data: {
          added: 1,
          results: [
            { source_id: 'trace-1', status: 'created', line_number: 0 },
            { source_id: 'trace-2', status: 'failed', error: 'Trace not found' },
          ],
        },
      })).not.toThrow();
    });
