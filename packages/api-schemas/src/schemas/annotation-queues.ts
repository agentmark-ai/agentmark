...
import { z } from "zod";
import { PaginationParamsSchema, itemResponse, listResponse, stripNullBytes } from "./common";
...
// Every user-supplied string field that lands in a Postgres TEXT column
// goes through `stripNullBytes` first — U+0000 would otherwise crash the
// INSERT with "unsupported Unicode escape sequence". See common.ts.
export const CreateAnnotationQueueBodySchema = z.object({
  ...
  score_config_names: z
    .array(z.preprocess(stripNullBytes, z.string()))
    .min(1, "at least one score config is required"),
  dataset_id: z.string().uuid().optional(),
  instructions: z.preprocess(stripNullBytes, z.string().max(5000)).optional(),
  reviewers_required: z.number().int().min(1).optional(),
});

export const UpdateAnnotationQueueBodySchema = z.object({
  name: z.preprocess(stripNullBytes, z.string().min(1).max(255)).optional(),
  description: z.preprocess(stripNullBytes, z.string().max(1000)).nullable().optional(),
  status: z.enum(ANNOTATION_QUEUE_STATUS_VALUES).optional(),
  score_config_names: z.array(z.preprocess(stripNullBytes, z.string())).nullable().optional(),
  instructions: z.preprocess(stripNullBytes, z.string().max(5000)).nullable().optional(),
  reviewers_required: z.number().int().min(1).optional(),
});
...
// `assigned_to_me=true` filters to queues with at least one pending assignment
// for the authenticated user (bearer auth). API-key auth returns empty per
// spec 052 §3.D / T072.
//
// Coercion note: `z.coerce.boolean()` is wrong here because it calls the
// JS `Boolean()` constructor, and `Boolean("false") === true` — so a
// caller passing `?assigned_to_me=false` would silently activate the
// filter. The preprocessor below maps the canonical string forms before
// reaching `z.boolean()`, so `=true`/`=false` round-trip correctly while
// SDK callers passing a native boolean still type-check.
export const AnnotationQueuesListParamsSchema = PaginationParamsSchema.extend({
  assigned_to_me: z
    .preprocess((val) => {
...
export type AnnotationQueuesListParams = z.infer<typeof AnnotationQueuesListParamsSchema>;
