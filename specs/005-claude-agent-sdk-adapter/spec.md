...
- What happens when tool execution times out? The adapter should respect timeout configurations and report timeouts appropriately.
- How does the system handle when Claude cannot complete a task within maxTurns? The agent stops at the limit and returns partial results with status indication.
- What happens when custom tools throw exceptions? The error is captured in telemetry and the agent can decide how to proceed.
- How does the adapter handle when MCP server connection fails? Connection failures are reported clearly and the agent operates with available tools only.
...
- **FR-003**: System MUST support configuring agent execution parameters (maxTurns, permissionMode, allowedTools, disallowedTools). When permissionMode is not specified, the system MUST default to 'default' mode (Claude Agent SDK's built-in permission prompts).
...
- **FR-013**: System MUST emit OpenTelemetry spans from hooks when telemetry is enabled, creating a root span on SessionStart and child spans for each tool call.
- **FR-014**: System MUST support configuring an OTLP exporter endpoint for span export. Users provide a pre-configured Tracer instance; the adapter emits spans to whatever tracer is provided.
...
- **FR-016**: System MUST properly end spans on corresponding end events (SessionEnd/Stop for root, PostToolUse/PostToolUseFailure for tool spans).
...
- **TelemetryHooks**: Configuration for capturing execution events and emitting OpenTelemetry spans.
- **TelemetryContext**: Context object for maintaining span hierarchy across hook callbacks.
...
- **SC-003**: All 8 telemetry event types (SessionStart, SessionEnd, PreToolUse, PostToolUse, PostToolUseFailure, Stop, SubagentStart, SubagentStop) are capturable.
