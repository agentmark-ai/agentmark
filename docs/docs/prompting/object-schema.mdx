...
The schema output is fully type-safe when using TypeScript. The returned object will match the structure defined in your schema.


Schema output is supported across multiple providers:

- OpenAI (gpt-3.5, gpt-4)
- Anthropic (claude-3)
- Ollama (compatible models)

Each provider may have slightly different implementations of schema support, so refer to their specific documentation for advanced features.
